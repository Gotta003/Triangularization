...
volatile int8_t current_rssi_A1=-127;
volatile int8_t current_rssi_A2=-127;
volatile int8_t current_rssi_A3=-127;
/* USER CODE END PV */
...
/* USER CODE BEGIN PFP2 */
static void Process_Advertising_Report(uint8_t *adv_report_data, uint8_t len_data, int8_t raw_rssi);
/* USER CODE END PFP2 */
...
/* USER CODE BEGIN PARSE_ADV_REPORT */
/**
 * @brief Parses one advertising report's AD structures, looking for either
 *        the phone's name ("MyPhone") or a dynamic anchor relay name
 *        ("A<id>-<rssi>"), and updates current_rssi_A1/A2/A3 accordingly.
 */
static void Process_Advertising_Report(uint8_t *adv_report_data, uint8_t len_data, int8_t raw_rssi)
{
  uint8_t i = 0;
  while (i < len_data)
  {
    ...
    if (ad_type == 0x09 || ad_type == 0x08)
    {
      ...
      if (strcmp(name_buf, "MyPhone") == 0)
      {
        ...
        current_rssi_A1 = (int8_t)filtered_rssi;
        //APP_DBG_MSG("A%d,%d", ANCHOR_ID, current_rssi_A1);
      }
      else if (name_buf[0] == 'A' && name_buf[1] != '\0')
      {
        /* Anchors format this as "A<id>:<rssi>", e.g. "A2:-70" */
        char* separator = strchr(name_buf, ':');
        if (separator != NULL && separator > &name_buf[1])
        {
          *separator = '\0';
          int parsed_id = atoi(&name_buf[1]);
          /* Take everything after the ':' as the signed RSSI text as-is
           * (e.g. "-70" -> -70). Using an explicit ':' delimiter means we
           * don't have to guess based on the number's own sign. */
          int8_t parsed_rssi = (int8_t)atoi(separator + 1);
          //APP_DBG_MSG("\r\n[DYNAMIC RX] Anchor ID: %d | Remote RSSI: %d\n\r", parsed_id, parsed_rssi);
          if (parsed_id == 2) {
            current_rssi_A2 = parsed_rssi;
          } else if (parsed_id == 3) {
            current_rssi_A3 = parsed_rssi;
          }
        }
        else
        {
          /* USER CODE BEGIN DEBUG_UNMATCHED_A */
          /* Starts with 'A' but no valid "-" separator found - log it raw so we
           * can see the actual format being advertised. */
          APP_DBG_MSG("\r\n[DEBUG] 'A'-prefixed name with no valid separator: \"%s\"\n\r", name_buf);
          /* USER CODE END DEBUG_UNMATCHED_A */
        }
      }
      else
      {
        /* USER CODE BEGIN DEBUG_UNMATCHED_NAME */
        /* Temporary diagnostic: print every other advertised name we see,
         * so we can confirm whether the anchors are advertising at all, and
         * if so, exactly what name string they're using. Remove once the
         * anchor format is confirmed. */
        APP_DBG_MSG("\r\n[DEBUG] Unmatched advertised name: \"%s\"\n\r", name_buf);
        /* USER CODE END DEBUG_UNMATCHED_NAME */
      }
    }
    i += ad_len + 1;
  }
}
/* USER CODE END PARSE_ADV_REPORT */
          ...
          case HCI_LE_ADVERTISING_REPORT_SUBEVT_CODE:
          		  {
          			...
          			int8_t physical_link_rssi = (int8_t)*(uint8_t*)(adv_report_data + len_data);

          			uint8_t i = 0;
          			char name_buf[32] = {0};
          			uint8_t phone_recognized = 0;

          			/* 1. Rilevamento rapido del telefono locale (A1) */
          			while (i < len_data)
          			{
          			  uint8_t ad_len = adv_report_data[i];
          			  if (ad_len == 0) break;
          			...
          			if (phone_recognized)
          			{
          			  ...
          			  static uint8_t first_sample = 1;
          			  if (first_sample) {
          				  filtered_rssi = (float)physical_link_rssi;
          				  first_sample = 0;
          			  }
          			  else {
          				  float alpha = 0.2f;
          				  filtered_rssi = (alpha * (float)physical_link_rssi) + ((1.0f - alpha) * filtered_rssi);
          			  }
          			  current_rssi_A1 = (int8_t)filtered_rssi;
          			}
          			/* 2. Decodifica universale dei dati di servizio P2P delle ancore */
          			else
          			{
          			  i = 0;
          			  while (i < len_data)
          			  {
          				...
          				/* Identifichiamo i Service Data (0x16) con UUID ST P2P (0xFE2C) */
          				if (ad_type == 0x16 && adv_report_data[i + 2] == 0x2C && adv_report_data[i + 3] == 0xFE)
          				{
          					/*APP_DBG_MSG("\r\n[P2P] RX payload: ");
							for (int j = 0; j <= ad_len; j++) {
								APP_DBG_MSG("%02X ", adv_report_data[i + j]);
							}
							APP_DBG_MSG("\n");*/
          				  /* Estraiamo l'ID e l'RSSI direttamente in base alla lunghezza del payload ricevuto */
          				  if (ad_len >= 12)
          				  {
          					// Se l'ancora ha inviato 13 byte (ad_len = 12), i nostri dati custom sono gli ultimi due byte
          					int parsed_id = (int)adv_report_data[i + ad_len - 1]; // Penultimo byte
          					int8_t extracted_rssi = (int8_t)adv_report_data[i + ad_len]; // Ultimo byte

          					if (parsed_id == 2 || parsed_id == 3)
          					{
          					  if (extracted_rssi < 0 && extracted_rssi > -120)
          					  {
          						//APP_DBG_MSG("\r\n[DYNAMIC RX] Anchor ID: %d | Remote Phone RSSI: %d\n\r", parsed_id, extracted_rssi);
          						if (parsed_id == 2) {
          						  current_rssi_A2 = extracted_rssi;
          						} else {
          						  current_rssi_A3 = extracted_rssi;
          						}
          					  }
          					}
          				  }
...
static void Scan_Request(void)
{
  ...
  if (BleApplicationContext.Device_Connection_Status != APP_BLE_CONNECTED_CLIENT)
  {
    ...
    /* USER CODE END BLE_SCAN_FAILED */
    	APP_DBG_MSG("-- Passive Observation Proc Failed: 0x%02X \r\n\r", result);
    }
  }
